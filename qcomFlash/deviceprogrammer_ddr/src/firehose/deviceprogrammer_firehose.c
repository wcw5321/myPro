 ...
 *   order.
 *
 *   $Header: //components/rel/boot.bf/3.3/boot_images/core/storage/tools/deviceprogrammer_ddr/src/firehose/deviceprogrammer_firehose.c#5 $ 
 *   $DateTime: 2017/03/17 11:06:47 $ $Author: pwbldsvc $
 *
 * YYYY-MM-DD   who     what, where, why
 * ----------   ---     ----------------------------------------------
 * 2016-10-05   wek     Add support for a partition erase
 * 2016-02-01   sb      Increase signed hash packet size for 4K signing
 * 2015-09-01   sb      Zeroing out the DDR ZI region for Crypto use
 * 2015-05-26   sb      Skip the check for storage drive in handleErase
...
#define FIREHOSE_SIGNED_HASH_PACKET_SIZE 12288
...
void deviceprogrammer_zi_buffers (void)
{
  ...
  SIZE_T  size;
    ...
    buf_start = (void *)SCL_DLOAD_BUFFERS_BASE;
    size = SCL_SBL1_DDR_BOOT_LOG_META_INFO_BASE - SCL_DLOAD_BUFFERS_BASE;
    memset (buf_start, 0, size);

    buf_start = (void *)&Image$$SBL1_DDR_ZI$$Base;
    size = (unsigned int)&Image$$SBL1_DDR_ZI$$ZI$$Length;
    memset (buf_start, 0, size);
#else
    buf_start = (void *)&Image$$SBL1_DEVICEPROGRAMMER_USB_ZI$$Base;
    size = (unsigned int)&Image$$SBL1_DEVICEPROGRAMMER_USB_ZI$$ZI$$Length;
    memset (buf_start, 0, size);
#endif
}
...
firehose_error_t handleErase() {
    ...
    if (FALSE == parseIntegerValue(start_sector_value, sizeof(start_sector_value), &start_sector)) {
       start_sector = (SIZE_T)-1; 
    }
    if (FALSE == parseIntegerValue(num_partition_sectors_value, sizeof(num_partition_sectors_value), &num_partition_sectors)) {
       num_partition_sectors = (SIZE_T)-1;
    }

    if ((start_sector == (SIZE_T)-1) || (num_partition_sectors == (SIZE_T)-1)) {
       logMessage("Erasing whole disk.");
       retval = storageDeviceFormat(&fh.emmc_storage);
    }
    else {
       logMessage("start "SIZE_T_FORMAT", num "SIZE_T_FORMAT, start_sector, num_partition_sectors);
       retval = storageDeviceErase(&fh.emmc_storage, start_sector, num_partition_sectors);
    }
...
firehose_error_t handleFirmwareProgram() {
	...
	//Removing this check as fh_loader.exe sends sector size as 1
