 ...
 *   order.
 *
 *   $Header: //components/rel/boot.bf/3.3/boot_images/core/storage/tools/deviceprogrammer_ddr/src/firehose/deviceprogrammer_initialize.c#3 $ 
 *   $DateTime: 2016/03/30 23:20:47 $ $Author: pwbldsvc $
 *
 * YYYY-MM-DD   who     what, where, why
 * ----------   ---     ----------------------------------------------
 * 2015-09-09   sb      RUMI specific changes
...
void deviceprogrammer_init_hw() {
    ...
    // initializes default values for fh structure
	...
	{ 
	  strlcat(err_log, "clock pre ddr:fail ", sizeof(err_log));
    }
#endif
    ...
    if(PM_ERR_FLAG__SUCCESS != pm_pon_channel_config())
	{
      strlcat(err_log, "init pmic:fail ", sizeof(err_log));
    }
#endif
    ...
    if (FALSE == initFirehoseStorage())
	{
      strlcat(err_log, "init storage:fail ", sizeof(err_log));
    }
